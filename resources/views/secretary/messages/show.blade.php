...
<style>
    ...
    .recipients-section { background:var(--cream); border:1px solid var(--border); border-radius:8px; padding:1.25rem; margin-top:1.5rem; }
    .recipients-section h4 { font-size:.9rem; color:var(--navy); margin-bottom:.75rem; text-transform:uppercase; letter-spacing:.05em; }
    .recipient-list { display:flex; flex-wrap:wrap; gap:.5rem; }
    .recipient-badge { display:inline-flex; align-items:center; gap:6px; padding:6px 12px; background:var(--white); border:1px solid var(--border); border-radius:20px; font-size:.8rem; }
    .recipient-badge.read { border-color:var(--green); color:var(--green); }
    .recipient-badge.unread { border-color:var(--gold); color:var(--gold); font-weight:600; }
...
<div class="page-header">
    ...
    <p class="reveal">Message Thread</p>
</div>
...
<div class="page-body">
    ...
    <div class="message-card reveal">
        <div class="message-header">
            ...
            <div class="message-meta">
                <span>📅 Sent {{ $message->created_at->format('M d, Y · H:i') }}</span>
                <span>👤 From: You</span>
                <span>📊 {{ $message->recipientCount() }} recipient(s)</span>
            </div>
        </div>
        <div class="message-body">{{ $message->body }}</div>

        <div class="recipients-section">
            <h4>Recipients ({{ $message->recipientCount() }})</h4>
            <div class="recipient-list">
                @foreach($message->recipients as $recipient)
                    <span class="recipient-badge {{ $recipient->read_at ? 'read' : 'unread' }}">
    ...
    @if($message->replies->isNotEmpty())
        <div class="replies-section reveal">
            <h3>💬 Replies ({{ $message->replies->count() }})</h3>
                    ...
                    <div class="reply-header">
                        <span class="reply-author">
                            @if($reply->sent_by === auth()->id())
                                You (Secretary)
                            @else
                                {{ $reply->sender->name }} ({{ $reply->sender->organization->name }})
                            @endif
                        </span>
    ...
    <div class="reply-form-card reveal">
        ...
        <form method="POST" action="{{ route('secretary.messages.reply', $message) }}">
            @csrf
